...
async function main() {
  ...
  const body = [
    "# UPlayground Events MCP Submission Evidence",
    "",
    ...
    `- MCP initialize: ${pass(checks.initialize?.ok)} (${checks.initialize?.server_info?.name || "unknown"})`,
    ...
    "## Expected Behavior",
    "",
    "- Direct event prompts return live UPlayground events with event URLs and ticket/source links when available.",
    ...
    "- Deletion calls `delete_event_preferences` only after explicit confirmation and is scoped to UPlayground connector preferences.",
    "",
