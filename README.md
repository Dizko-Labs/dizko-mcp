...
| `EVENTCHAT_MCP_ALLOWED_ORIGINS` | `*` | Comma list for `Access-Control-Allow-Origin`. When set to anything but `*`, a request carrying an `Origin` outside the list is refused with 403 rather than only being denied by the browser. Setting it to an empty or blank value denies every browser origin, since clearing an allowlist means locking down, not opening up. Requests with no `Origin` (every non-browser client) are unaffected. |
